...
import { Trash2, Film, Radio, Layers } from "lucide-react";
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between px-1">
        ...
        <span className="text-xs text-slate-500">Customize viewing style per show</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
        {shows.map((show) => {
          ...
          return (
            <div
              key={show.id}
              className="bg-slate-900/90 border border-slate-800 hover:border-slate-700/80 rounded-xl p-4 transition-all shadow-md flex flex-col justify-between"
            >
              <div className="flex items-start justify-between space-x-3">
                <div className="flex space-x-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h4 className="font-bold text-slate-100 text-sm truncate">{show.title}</h4>
                    ...
                    {premiere && finale && (
                      <p className="text-[11px] text-slate-400 mt-1">
                        {premiere === finale ? `Airs: ${premiere}` : `${premiere} &rarr; ${finale}`}
                      </p>
                    )}
                  </div>
                </div>
